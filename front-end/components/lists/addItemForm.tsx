...
type Props = {
    ...
    onNeedRefresh: () => void;
    onCreateList?: (ToAddListName:string) => void;
    shoppingListName: string;
};

const AddItemForm: React.FC<Props> = ({onCreateList, onAddItem, onNeedRefresh, shoppingListName}) => {
    ...
    const handleCreateList = async (e: React.FormEvent) => {
        if (onCreateList) {
            e.preventDefault();
            if (toAddListName.trim() === '') {
                setErrorMessage(t('lists.form.validate.listname'));
                return;
            }
            setErrorMessage(null);
            onCreateList(toAddListName);
            setListName(toAddListName);
            setToAddListName('');
            await onNeedRefresh();
        }
    };
    ...
    return (
        <div className="flex flex-col items-center p-4">
            {listName === '' ? (
                <p className="text-lg font-medium mb-4">{t("lists.form.noList")}</p>
            ) : (
                <p className="text-lg font-medium mb-4">{t("lists.form.title")}: {listName}</p>
            )}
            {errorMessage && <p className="text-red-500 mb-4">{errorMessage}</p>}
            <form onSubmit={listName === '' ? handleCreateList : handleAddItem} className="w-full max-w-lg bg-transparent p-6 rounded-lg shadow-md">
                {listName.trim() === '' && (
                    <div className="mb-4">
                ...
                <div className="mb-4">
                    <button
                        type="submit"
                        disabled={listName !== '' && !isFormValid}
                        className="w-full bg-blue-500 text-white px-3 py-2 rounded disabled:opacity-50"
                    >
                        {listName === '' ? t("lists.form.createList") : t("lists.form.submit")}
                    </button>
                </div>
            </form>
        </div>
